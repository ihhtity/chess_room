...
import { Table, Button, Modal, Form, Input, DatePicker, Select, message, Tag, Checkbox } from 'antd'
...
export default function HolidayManage() {
  ...
  const fetchData = async (params?: Record<string, string>, page: number = currentPage, size: number = pageSize) => {
    try {
      const result = await holidayApi.getList({ ...params, page: String(page), page_size: String(size) })
  ...
  const handleBatchDelete = async (ids: string[]) => {
    try {
      await holidayApi.batchDelete(ids.map(id => parseInt(id)))
  ...
  const handleBatchEdit = (ids: string[]) => {
    const selected = data.filter(h => ids.includes(String(h.id)))
  ...
  const handleBatchEditSubmit = async (updatedRecords: Record<string, any>[]) => {
    try {
      await holidayApi.batchUpdate(updatedRecords)
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <ExportDropdown data={data} filename="节假日管理数据" />
          <BatchActions
      ...
      <BatchEditModal
        ...
        records={selectedHolidays}
        fields={[
          { key: 'name', label: '节假日名称', type: 'input' },
          { key: 'is_holiday', label: '类型', type: 'select', options: [{ label: '节假日', value: 1 }, { label: '工作日', value: 0 }] },
