...
func main() {
	...
	go func() {
		...
		if viper.GetString("server.env") == "production" {
			swaggerURL = "https://api.ihhtity.icu/api/swagger/index.html"
		} else {
			swaggerURL = fmt.Sprintf("http://localhost:%s/swagger/index.html", viper.GetString("server.port"))
		}
		// 记录 Swagger 文档地址
		log.Logger.Info("API文档地址: " + swaggerURL)
		// 启动服务并监听端口
